-- =============================================================================
-- HGAE — Partner-Portal Schema (partner_profiles + RLS)
-- =============================================================================
...
create index if not exists partner_profiles_channel_id_idx
  on public.partner_profiles (channel_id);
...
drop policy if exists "Partners can read linked hotels" on public.hotels;
drop policy if exists "Staff can read channels" on public.channels;
drop policy if exists "Partners can read own channels" on public.channels;
drop policy if exists "Staff can read touchpoints" on public.touchpoints;
drop policy if exists "Partners can read own touchpoints" on public.touchpoints;
...
create policy "Partners can read own channels"
  ...
  using (
    exists (
      ...
      where pp.user_id = auth.uid() and pp.channel_id = channels.id
    )
  );
...
create policy "Partners can read own touchpoints"
  ...
  using (
    exists (
      select 1 from public.partner_profiles pp
      where pp.user_id = auth.uid() and pp.channel_id = touchpoints.channel_id
    )
  );
...
create policy "Partners can read own bookings"
  ...
  using (
    exists (
      select 1 from public.partner_profiles pp
      where pp.user_id = auth.uid() and pp.channel_id = bookings.channel_id
    )
  );
